function MobileFriendlyCursor( settingsObject ){
  let thisCopy = this;
  ...
  if(typeof settingsObject == "object"){
    ...
    if(this_settings_keys.includes(i)){
      ...
      console.warn("Warning: MobileFriendlyCursor setting " + i + " should be of type " + typeof this.settings[i] + " instead of type " + typeof settingsObject[i])
      else {
        let isBetween = (n, a, b) => {return n >= a && n <= b}
        if(i == "swipeDeceleration" && !isBetween(settingsObject[i], 0, 1) )
        console.warn("Warning: MobileFriendlyCursor setting swipeDeceleration should be a value between 0 and 1")
        if(i == "swipeOnButton" && !["left","right","middle","any","none"].includes(settingsObject[i]) )
        console.warn("Warning: MobileFriendlyCursor setting swipeOnButton should contain one of the following: left, right, middle, any, none")
      }
      ...
      console.warn(i + " is not an MobileFriendlyCursor setting. Did you mean " + closestMatch + "?")
    }
  }
  } else if(typeof settingsObject !== "undefined") {
    console.warn("Warning: MobileFriendlyCursor takes an object as a parameter for settings. You have passed it a(n) " + typeof settingsObject)
  }
  ...
  this.update = function() {
    ...
    if(frameCount - _timeOfLastSwipeUpdate > 1 &&
      ...
      console.warn("Warning: It appears the update() method of your MobileFriendlyCursor is not being called every frame. Be sure to put it in draw()")
  ...
  this.render = function(){
    ...
    noFill();
    rect(x, y, w, w)
    if(thisCopy.leftPressed)fill(color("green")); else noFill();
    rect(x, y, w/2, w/2)
    if(thisCopy.rightPressed)fill(color("red")); else noFill();
    rect(x + (w/2), y, w/2, w/2)
    if(thisCopy.middlePressed)fill(color("blue")); else fill(0);
